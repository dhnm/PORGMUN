...
class NewsViewController: UITableViewController, TTTAttributedLabelDelegate, SFSafariViewControllerDelegate, MFMailComposeViewControllerDelegate {
                                        ...
                                        if let fetchAlbumURLSafe = fetchAlbumURL {
                                            URLSession.shared.dataTask(with: fetchAlbumURLSafe) { (data, response, error) in
                                                ...
                                                if let albumJSON = data {
                                                    do {
                                                        let albumJsonResult = try JSONSerialization.jsonObject(with: albumJSON, options: JSONSerialization.ReadingOptions.mutableContainers) as AnyObject
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let _ = message {
            ...
            cell.messageLabel.setText(text, afterInheritingLabelAttributesAndConfiguringWith: {(_ mutableAttributedString: NSMutableAttributedString?) -> NSMutableAttributedString in
